...
export default function CancelGroupButton({ groupId }: { groupId: string }) {
  const router = useRouter();
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  const [armed, setArmed] = useState(false);

  // cancel the session and refund every enrolled student
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      >
        {busy ? "Cancelling…" : armed ? "Confirm cancel" : "Cancel session"}
      </button>
